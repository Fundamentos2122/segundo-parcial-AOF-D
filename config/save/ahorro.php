...
if (isset($_POST['id'])) {
    ...
    $query = $con->prepare("UPDATE  ahorros SET concepto=?, ahorrado=?, objetivo=?  WHERE id=?");
    $resultado = $query->execute(array($concepto, $ahorrado, $objetivo, $id));
    ...
    $query = $con->prepare("INSERT INTO ahorros (concepto, ahorrado, objetivo, activo) VALUES (:conc, :aho, :obj, 1)");
    $resultado = $query->execute(array('conc' => $concepto, 'aho' => $ahorrado, 'obj' => $objetivo));
